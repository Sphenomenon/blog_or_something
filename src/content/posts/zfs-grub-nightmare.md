...
新系统很听话，我们和平相处了两周。昨天下午开机，熟悉的画面又出现了。

叒叕是同样的报错。

好在这次没设密钥。我在 live USB 中挂载了 pool，翻完日志和资料，把问题定位到 GRUB：
...
稳妥的做法是换用支持 ZFS 的引导器，或者调整 pool 的 feature flags。但我当时没看到那篇文章，最后用了更野的办法：直接破坏 GRUB 配置，让引导流程 fallback 到 systemd-boot，绕过 GRUB 的算法问题。

> 别问为什么不用正解。问就是我倒腾完了才看见那篇正确的文章。

systemd-boot 只能访问 EFI 分区，坑还没填完。如果不同步 zpool 里的内核文件，新旧内核割裂太久，旧内核会被系统判定为损坏，开机时便又回到最初的报错。
